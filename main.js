...
function updateDragonCount() {
  document.getElementById("numdragon").innerHTML = formatEggs(_premiumDragons * eggstohatch1);
}

function updatedProductionCount() {
  var productiondoc = document.getElementById('production');
  productiondoc.textContent = formatEggs((_premiumDragons * 60 * 60) + (_myIceDragons * 1 * 60 * 60 * 10) + _myUltraDragons * 1 * 60 * 60 * 20);
}
...
function updateIceDragonCount() {
  ...
  if (_myIceDragons > 0) {
    ...
    document.getElementById("numdragonice").innerHTML = formatEggs(_myIceDragons * eggstohatch1);;
  } else {
...
function updateUltraDragonCount() {
  var not = document.getElementById("ultraDragonNotBought");
  var bought = document.getElementById("ultraDragonBought");
  if (_myUltraDragons > 0) {
    bought.style.display = "block";
    not.style.display = "none";
    document.getElementById("numdragonultra").innerHTML = formatEggs(_myUltraDragons * eggstohatch1);
  } else {
    not.style.display = "block";
    bought.style.display = "none";
  }
...
function updateBNB_per_hour() {
  ...
  getDragonSellPrice(eggs1, function (price) {
	...
	const production1 = formatEggs((_premiumDragons * 60 * 60) + (_myIceDragons * 1 * 60 * 60 * 10) + _myUltraDragons * 1 * 60 * 60 * 25);
...
function updateSellForText() {
  ...
  getDragonSellPrice(eggs, function (price) {
    ...
    sellsforexampledoc.textContent = '(Dispersed ki ' + formatEggs(sellFor) + ' yields roughly ' + formatEthValue(1) + ' BNB)';
  });
}
...
function updateDragonButton() {
  ...
  getNumberOfDragons(eth, function (dragons) {
    // alert(eth, amount);
    dragons = dragons * 0.95;
    premium.innerHTML = "Train " + formatDragons(dragons) + " hours <br>for " + parseFloat(premiumInput.value).toFixed(4) + " BNB";
  });
...
function buyUltraDragonInput() {
  var eth = parseFloat(document.getElementById("buyUltraDragonInput").value) * 1000000 * 1000000 * 1000000;
  buyUltraDragon(eth, displayTransactionCompletedMessage);
}

function updateIceDragonPrice() {
  // document.getElementById("boughtIceDragons").innerHTML = formatEggs(_myIceDragons * eggstohatch1);;
  var eth = document.getElementById("buyIceDragonInput").value;
  var eth = parseFloat(eth) * 1000000 * 1000000 * 1000000
  getNumberOfDragons(eth, function (dragons) {
    dragons = dragons * 0.95;
    document.getElementById("buyIceDragon").innerHTML = "Train " + formatDragons(dragons / 9) + " hours <br>for " + parseFloat(document.getElementById("buyIceDragonInput").value).toFixed(2) + " BNB";
  });
}

function updateUltraDragonPrice() {
  // document.getElementById("boughtUltraDragons").innerHTML = _myUltraDragons;
  var eth = document.getElementById("buyUltraDragonInput").value;
  var eth = parseFloat(eth) * 1000000 * 1000000 * 1000000
  getNumberOfDragons(eth, function (dragons) {
    dragons = dragons * 0.95;
    document.getElementById("buyUltraDragon").innerHTML = "Train " + formatDragons(dragons / 17) + " hours <br>for " + parseFloat(document.getElementById("buyUltraDragonInput").value).toFixed(2) + " BNB";
  });
}
...
function refreshData() {
  ...
  updateUltraDragonCount();

  updateSellForText();
  const d = (_premiumDragons + _myIceDragons * 10 + _myUltraDragons * 20);
  if (d === 0) {
...
function setRef() {
  ...
  document.getElementById("playerreflink").innerHTML = 'http://DragonBallFarmz.Finance/?ref=' + this.userAddress;
}
